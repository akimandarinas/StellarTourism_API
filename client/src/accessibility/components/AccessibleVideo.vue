<template>
  <div 
    :class="[containerClass, 'accessible-video-container']" 
    :style="containerStyle"
    ref="container"
  >
    <!-- Título accesible para el video -->
    ...
    <div class="video-wrapper">
      <video
        ...
        :aria-label="title || ariaLabel || 'Video'"
      ...
      <div v-if="customControls" class="accessible-video-controls" aria-hidden="true">
        <div class="accessible-video-progress">
          <div 
            class="accessible-video-progress-bar"
            @click="seekToPosition"
            ref="progressBar"
          >
            <div 
              class="accessible-video-progress-fill" 
              :style="{ width: `${progress}%` }"
            ></div>
          </div>
          <div class="accessible-video-time">
        ...
        <div class="accessible-video-buttons">
          <button 
            @click="togglePlay"
            class="accessible-video-button"
          ...
          <div class="accessible-video-volume">
            <input 
              type="range" 
              min="0" 
              max="1" 
              ...
              class="accessible-video-volume-slider"
              aria-label="Volumen"
              :aria-valuemin="0"
              :aria-valuemax="100"
          ...
          <button 
            ...
            class="accessible-video-button"
            :class="{ 'accessible-video-button-active': subtitlesEnabled }"
    ...
    <div 
      v-if="description" 
      class="accessible-video-description"
      :id="descriptionId"
    >
      <p>{{ description }}</p>
    </div>
    ...
    <div v-if="transcript || transcription" class="accessible-video-transcript">
      <button 
        @click="toggleTranscript"
        class="accessible-video-transcript-toggle"
      ...
      <div 
        ...
        class="accessible-video-transcript-content"
        :id="transcriptId"
      >
        <h4 class="accessible-video-transcript-title">Transcripción</h4>
    ...
    <div 
      v-if="error" 
      class="accessible-video-error"
      role="alert"
    >
      <slot name="error">
        <div class="accessible-video-error-content">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="accessible-video-error-icon" aria-hidden="true">
...
export default {
  ...
  emits: ['play', 'pause', 'ended', 'timeupdate', 'volumechange', 'error', 'loaded'],
  ...
  setup(props, { emit }) {
    ...
    // Usar el composable de anunciador
    const { announce } = useAnnouncer();

    // Generar IDs únicos para accesibilidad
    const uniqueId = generateId('video');
    ...
    // Usar transcript o transcription (para compatibilidad con ambas versiones)
    const effectiveTranscript = computed(() => {
      return props.transcript || props.transcription || '';
    });

    // Formatear transcripción con saltos de línea
    const formattedTranscript = computed(() => {
      if (!effectiveTranscript.value) return '';
      return effectiveTranscript.value.replace(/\n/g, '<br>');
    });
...
.accessible-video-container:hover .accessible-video-controls,
.accessible-video-container:focus-within .accessible-video-controls {
  opacity: 1;
}

.accessible-video-progress {
  display: flex;
  align-items: center;
  margin-bottom: 0.5rem;
}

.accessible-video-progress-bar {
  flex: 1;
  height: 4px;
...
.accessible-video-progress-fill {
  height: 100%;
  background-color: var(--color-primary, #3182ce);
  border-radius: 2px;
}

.accessible-video-time {
  color: white;
  font-size: 0.75rem;
  min-width: 80px;
  text-align: right;
...
.accessible-video-button:focus-visible {
  outline: 2px solid white;
  outline-offset: 2px;
}

.accessible-video-button-active {
  color: var(--color-primary, #3182ce);
}

.accessible-video-volume {
  display: flex;
  align-items: center;
  margin-right: 0.5rem;
}

.accessible-video-volume-slider {
  width: 80px;
  height: 4px;
...
.accessible-video-volume-slider::-webkit-slider-thumb {
  -webkit-appearance: none;
  appearance: none;
  width: 12px;
  height: 12px;
...
.accessible-video-volume-slider::-moz-range-thumb {
  width: 12px;
  height: 12px;
  border-radius: 50%;
  background-color: white;
...
.accessible-video-transcript-toggle {
  background: none;
  border: none;
  color: var(--color-primary, #3182ce);
  font-size: 0.875rem;
...
.accessible-video-transcript-toggle:hover,
.accessible-video-transcript-toggle:focus {
  color: var(--color-primary-dark, #2c5282);
  background-color: var(--color-background-hover, #f7fafc);
}

.accessible-video-transcript-toggle:focus-visible {
  outline: 2px solid var(--color-primary, #3182ce);
  outline-offset: 2px;
}

.accessible-video-transcript-content {
  margin-top: 0.5rem;
  padding: 0.75rem;
...
.accessible-video-transcript-title {
  font-size: 1rem;
  font-weight: 600;
  margin: 0 0 0.5rem;
}
...
.accessible-video-error-content {
  display: flex;
